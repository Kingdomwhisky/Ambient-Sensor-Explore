...
#if((_HP_SOURCE_SCAN_SUPPORT == _ON) && (_TWO_CHIP_DATA_EXCHANGE_MODE != _DATA_EXCHANGE_MODE_NONE))
...
void SysSourceSearchTimeInitial(BYTE ucSearchIndex)
{
...
#endif
        //DebugMessageSystem("GET_SOURCE_SEARCH_DELAY_MODE():", GET_SOURCE_SEARCH_DELAY_MODE());
    ...
    else if(GET_SOURCE_SEARCH_DELAY_MODE() == _SOURCE_SEARCH_DELAY_REACTIVE_MODE)
    {
        SET_SOURCE_SEARCH_DELAY_MODE(_SOURCE_SEARCH_DELAY_DEFAULT_MODE);
                //DebugMessageSystem("SourceTimeInitialporttype", GET_PORT_TYPE(GET_INPUTPORT_TYPE(ucSearchIndex)));
    ...
    else
    {
        switch(GET_SOURCE_SCAN_TYPE())
        {
            ...
            case _SOURCE_SWITCH_DELL_HOTKEY_AUTO:
                ...
                //DebugMessageSystem("_SOURCE_AUTO_SEARCH_STYLE", GET_PORT_TYPE(GET_INPUTPORT_TYPE(ucSearchIndex)));
#else
                switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(ucSearchIndex)))
                {
                //DebugMessageSystem("SourceTimeInitialPORT_TYPe", GET_PORT_TYPE(GET_INPUTPORT_TYPE(ucSearchIndex)));
...
void SysSourceJudge(void)
{
    ...
    switch(GET_SOURCE_SCAN_TYPE())
    {
        ...
        case _SOURCE_SWITCH_HP_SCAN_NEXT_PORT:
            ...
            if(GET_SOURCE_TYPE(g_ucSearchIndex) != _SOURCE_NONE)
            {
#if(_TWO_CHIP_DATA_EXCHANGE_MODE != _TWO_CHIP_DATA_EXCHANGE_MODE)
                CLR_SOURCE_HP_DETECTING();
#endif
                SET_SOURCE_SEARCH_DELAY_MODE(_SOURCE_SEARCH_DELAY_DEFAULT_MODE);
    ...
    switch(GET_SOURCE_SCAN_TYPE())
    {
        case _SOURCE_SWITCH_FIXED_PORT:
            ...
            if(GET_SOURCE_SEARCH_TIMES(g_ucSearchIndex) == 0x00)
            {
                ...
                SET_SOURCE_SWITCH_FINISH();
                //DebugMessageSystem("_SOURCE_SWITCH_FIXED_PORT", _SOURCE_SWITCH_FIXED_PORT);
            }
...
#endif // End of #if(_SOURCE_AUTO_SEARCH_STYLE == _SOURCE_POLLING_IN_SEARCH_STATE)
                DebugMessageSystem("SysSourceSwitchPort", _SOURCE_SWITCH_AUTO_IN_GROUP);
...
void SysSourceScanPortInitial(BYTE ucSearchIndex)
{
    ...
    // USB HUB switch follow source port
    switch(enumSearchPort)
    {
        ...
        case _D6_INPUT_PORT:
#endif
            if(PCB_USBHUB_CHN_SEL_DETECT() != _LOW)
            {
                PCB_USBHUB_CHN_SEL(_LOW);
                DebugMessageSystem("D0. USBHUB", PCB_USBHUB_CHN_SEL_DETECT());
            }                        
        ...
        case _D1_INPUT_PORT:
           /* if(PCB_USBHUB_CHN_SEL_DETECT() != _HIGH)
            {
                PCB_USBHUB_CHN_SEL(_HIGH);
                DebugMessageSystem("D1. USBHUB", PCB_USBHUB_CHN_SEL_DETECT());
            }                        
            */
            break;
#endif
        ...
        case _D2_INPUT_PORT:
            ...
            {
                PCB_USBHUB_CHN_SEL(_HIGH);
                DebugMessageSystem("D2. USBHUB", PCB_USBHUB_CHN_SEL_DETECT());
            }
...
void SysSourceInfoReset(void)
{
...
#endif
    {
        ...
        {
            SET_SOURCE_SEARCH_DELAY_MODE(_SOURCE_SEARCH_DELAY_DEFAULT_MODE);
        DebugMessageSystem("SysSourceInfoReset", 1);
