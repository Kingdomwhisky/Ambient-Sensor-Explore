...
StructSystemDataType code g_stSystemDefaultData =
{
    ...
    _SOURCE_SWITCH_FIXED_PORT,         // Source Scan Type
#else
    ...
    _OFF,                              // BYTE b3SensorCalibrationMode
    _DEFAULT_POWER_ON_LOGO,            // BYTE b1DisplayLogoMode
    _OFF,                              // BYTE b2LightboxModeType
    _ON,                               // BYTE b1OGCOCCSelectMode:On->Beacon off->RTD
    _OFF,                              // BYTE b1GammaNativeMode
    _DEFAULT_KEYLOCK,                  // BYTE b1KeyLockStatus
    _OFF,                              // BYTE b1OneKeyPlusLuminanceDisplay
    0x01,                              // BYTE b3FPGAColorAdaptiveThreshold    
    _OFF,                              // BYTE b1FPGALocalEnhancementMode
    
    _OFF,                              // BYTE ucTestImageBackupDisplayMode
...
BYTE UserCommonNVRamGetSystemData(EnumSystemDataItemDataType enumDataType)
{
    ...
    ((_HDMI_FREESYNC_SUPPORT == _ON) || (_TMDS_HDCP_2_2_SUPPORT == _ON)))
        ...
        case _SENSOR_CALIBRATION_MODE:
            ucValue = g_stSystemData.b3SensorCalibrationMode;
            break;
        case _DISPLAY_LOGO_MODE:
            ucValue = g_stSystemData.b1DisplayLogoMode;
            break;
        case _FPGA_TEST_IMAGE_BACKUP_DISPLAY_MODE:
            ucValue = g_stSystemData.ucTestImageBackupDisplayMode;
            break;
        case _LIGHTBOX_MODE_TYPE:
            ucValue = g_stSystemData.b2LightboxModeType;
            break;
        case _OGCOCC_SEL_BEACON:
            ucValue = g_stSystemData.b1OGCOCCSelectMode;
            break;

        case _GAMMA_NATIVE_SET_MODE:
            ucValue = g_stSystemData.b1GammaNativeMode;
            break;

        case _KEY_LOCK_STATUS:
            ucValue = g_stSystemData.b1KeyLockStatus;
            break;
            
        case _ONEKEY_PLUS_LUMINANCE_DISPLAY_MODE:
            ucValue = g_stSystemData.b1OneKeyPlusLuminanceDisplay;
            break;
        case _FPGA_COLOR_ADAPTIVE_THRESHOLD:
            ucValue = g_stSystemData.b3FPGAColorAdaptiveThreshold;
            break;
			
        case _FPGA_LOCAL_ENHANCEMENT_MODE:
            ucValue = g_stSystemData.b1FPGALocalEnhancementMode;
            break;
...
void UserCommonNVRamSetSystemData(EnumSystemDataItemDataType enumDataType, BYTE ucValue)
{
    switch(enumDataType)
    {
        ...
        case _SENSOR_CALIBRATION_MODE:
            g_stSystemData.b3SensorCalibrationMode = ucValue;
            break;
        case _DISPLAY_LOGO_MODE:
            g_stSystemData.b1DisplayLogoMode = ucValue;
            break;
        case _FPGA_TEST_IMAGE_BACKUP_DISPLAY_MODE:
            g_stSystemData.ucTestImageBackupDisplayMode = ucValue;
            break;
        case _LIGHTBOX_MODE_TYPE:
            g_stSystemData.b2LightboxModeType = ucValue;
            break;
        case _OGCOCC_SEL_BEACON:
            g_stSystemData.b1OGCOCCSelectMode = ucValue;
            break;
			
        case _GAMMA_NATIVE_SET_MODE:
            g_stSystemData.b1GammaNativeMode = ucValue;
            break;

        case _KEY_LOCK_STATUS:
            g_stSystemData.b1KeyLockStatus = ucValue;
            break;
            
        case _ONEKEY_PLUS_LUMINANCE_DISPLAY_MODE:
            g_stSystemData.b1OneKeyPlusLuminanceDisplay = ucValue;
            break;
        case _FPGA_COLOR_ADAPTIVE_THRESHOLD:
            g_stSystemData.b3FPGAColorAdaptiveThreshold = ucValue;
            break;

        case _FPGA_LOCAL_ENHANCEMENT_MODE:
            g_stSystemData.b1FPGALocalEnhancementMode = ucValue;
            break;
...
#if 1//(_SW_IIC_SUPPORT == _ON)
        if(ucSWIICStatus == _IIC_SYS_EEPROM)
#endif
        {
...
{
    BYTE tempDclkSSC;
    tempDclkSSC = UserCommonNVRamGetSystemData(_SSC);

    g_stSystemData = g_stSystemDefaultData;
    if(GET_FORCE_BACKUP_STATUS() == _TRUE)
	{
        UserCommonNVRamSetSystemData(_SSC,tempDclkSSC);	
    }   
