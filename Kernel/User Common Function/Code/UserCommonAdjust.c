...
BYTE UserCommonAdjustPanelUniformityGainBank(EnumUniformityTypeSelect enumUniformityType)
{
    ...
    for(ucUniformityTypeNum = 0; ucUniformityTypeNum <= enumUniformityType; ucUniformityTypeNum++)
    {
 	...
 	DebugMessageSystem("UniGainBank:", ucUniformityTypeBank + _UNIFORMITY_FLASH_BANK0);
...
WORD UserCommonAdjustPanelUniformityGainAddr(EnumUniformityTypeSelect enumUniformityType)
{
    ...
    for(ucUniformityTypeNum = 0; ucUniformityTypeNum <= enumUniformityType; ucUniformityTypeNum++)
    {
        usUniformityTableSize = ((DWORD)_PANEL_UNIFORMITY_1LEVEL_SIZE * UserCommonAdjustGetUniformityType(ucUniformityTypeNum) + 1);
     	DebugMessageSystem("GetUniType:", UserCommonAdjustGetUniformityType(ucUniformityTypeNum));
     	DebugMessageSystem("UniSize:", usUniformityTableSize);
        if(ulRemainder < usUniformityTableSize)
        {
            if(ucQuotient == 0)
            {
        ...
        {
            if(usUniformityTableSize == 1)
            {
                // Address unchanged: ->Uniformity type level not defined:tUniformityType
            }
            else
            {
        ...
        ulRemainder = (ulRemainder - usUniformityTableSize);
     	//DebugMessageSystem("---ulRemainder---:", ulRemainder);
    }
 	DebugMessageSystem("---UniGainAddr---:", usUniformityTypeAddr);
...
// Output Value : None
//--------------------------------------------------
BYTE UserCommonAdjustOGCTypeBeacon(EnumOGCTypeSelect enumOGCTypeSelect)
{
    enumOGCTypeSelect = enumOGCTypeSelect;
 	//DebugMessageUart("In OGCTypeSelect:", enumOGCTypeSelect);
	...
	if(UserCommonNVRamGetSystemData(_OGCOCC_SEL_BEACON) == _ON)
    {
        ...
        if(GET_OSD_GAMMA() == _GAMMA_18)
        {
            ...
            {
                if(GET_USER_BASE_COLOR_TEMP_TYPE() == _CT_9300)
                {
                    enumOGCTypeSelect = _OGC_GAMMA1_TYPE;
                }
                else// if(GET_USER_BASE_COLOR_TEMP_TYPE() == _CT_6500)
                {
                    enumOGCTypeSelect = _OGC_GAMMA21_TYPE;
                }
            }
            else if(GET_COLOR_TEMP_TYPE() < _CT_6500)
            {
        ...
        else if(GET_OSD_GAMMA() == _GAMMA_20)  // DICOM or DICOM+ALC  (IsGammaOptionDICOMALC(GET_OSD_GAMMA()))
        {
            if(GET_COLOR_TEMP_TYPE() == _CT_USER)
            {
                if(GET_USER_BASE_COLOR_TEMP_TYPE() == _CT_9300)
                {
                    enumOGCTypeSelect = _OGC_GAMMA2_TYPE;

                    if(GET_OSD_DICOMALC()/(_ALC_AUTO) == _TRUE)
                    {
                        // == _ALC_AUTO
                        enumOGCTypeSelect = _OGC_GAMMA2_TYPE + UserGetDICOMALCAmbiSensor();
                    }
                    else
                    {
                        // <  _ALC_AUTO
                        enumOGCTypeSelect = _OGC_GAMMA2_TYPE + (GET_OSD_DICOMALC()%(_ALC_AUTO));
                    }
                }
                else// if(GET_USER_BASE_COLOR_TEMP_TYPE(ucOSDSelectRegion) == _CT_6500)
                {
                    enumOGCTypeSelect = _OGC_GAMMA22_TYPE;

                    if(GET_OSD_DICOMALC()/(_ALC_AUTO) == _TRUE)
                    {
                        // == _ALC_AUTO
                        enumOGCTypeSelect = _OGC_GAMMA22_TYPE + UserGetDICOMALCAmbiSensor();
                    }
                    else
                    {
                        // <  _ALC_AUTO
                        enumOGCTypeSelect = _OGC_GAMMA22_TYPE + (GET_OSD_DICOMALC()%(_ALC_AUTO));
                    }
                }
            }
            else if(GET_COLOR_TEMP_TYPE() < _CT_6500)
            ...
            // DICOM Auto: Auto Switch Ambientlight Sensor On/off
            {
            	static bit ucAutoChanged = 0;
                if(GET_OSD_DICOMALC()/(_ALC_AUTO) == _TRUE)
                {
                    // == _ALC_AUTO
                    if(GET_OSD_AMBIENTLIGHT_SENSOR_STATUS()== _OFF)
                    {
                        SET_OSD_AMBIENTLIGHT_SENSOR_STATUS(_ON);
                ...
                {
                    // <  _ALC_AUTO
                    if((GET_OSD_AMBIENTLIGHT_SENSOR_STATUS()== _ON)&&(ucAutoChanged))
                    {
                        SET_OSD_AMBIENTLIGHT_SENSOR_STATUS(_OFF);
                    	ScalerTimerReactiveTimerEvent(SEC(2), _USER_TIMER_EVENT_AMBI_SENSOR_ADC_START);	 
                        ucAutoChanged = _FALSE;
                    }
                }
            }
#endif
         	DebugMessageUart("DicomALC/:", GET_OSD_DICOMALC()/(_ALC_AUTO));
         	DebugMessageUart("DicomALC%:", GET_OSD_DICOMALC()%(_ALC_AUTO));
        }
        else if(GET_OSD_GAMMA() == _GAMMA_22)  // User  (IsGammaOptionMCUCalibration(GET_OSD_GAMMA()))
        {
            if(GET_COLOR_TEMP_TYPE() == _CT_USER)
            {
                if(GET_USER_BASE_COLOR_TEMP_TYPE() == _CT_9300)
                {
                    ...
                    {
                        enumOGCTypeSelect = _OGC_GAMMA32_TYPE;
                    }
                    else
                    {
                        enumOGCTypeSelect = _OGC_GAMMA34_TYPE;
                    }
                }
                else// if(GET_USER_BASE_COLOR_TEMP_TYPE(ucOSDSelectRegion) == _CT_6500)
                {
                    if(GET_OSD_LUT_USER_TYPE() == _LUT_USER_DICOM)
                    {
                        enumOGCTypeSelect = _OGC_GAMMA33_TYPE;
                    }
                    else
                    {
                        enumOGCTypeSelect = _OGC_GAMMA35_TYPE;
                    }
                }
            }
            else if(GET_COLOR_TEMP_TYPE() < _CT_6500)
            ...
            else if(GET_COLOR_TEMP_TYPE() >= _CT_6500)
            {
         	...
         	DebugMessageUart("MCU LUT:", GET_OSD_LUT_USER_TYPE());
        }
        else if(GET_OSD_GAMMA() == _GAMMA_24) // Calib  (IsGammaOptionPCCalibration(GET_OSD_GAMMA()))
        {
            enumOGCTypeSelect = _OGC_GAMMA31_TYPE;
        }
        
#elif (_GAMMA_TYPE == _GAMMA_NAME_G20_G22_G24_G26_DICOM_CALIB)
        {
            if(GET_OSD_GAMMA() == _GAMMA_26) // DICOM or DICOM+ALC  (IsGammaOptionDICOMALC(GET_OSD_GAMMA()))
            {
                if(GET_COLOR_TEMP_TYPE() == _CT_USER)
                {
                    if(GET_USER_BASE_COLOR_TEMP_TYPE() == _CT_9300)
                    {
                        enumOGCTypeSelect = _OGC_GAMMA5_TYPE;

                        if(GET_OSD_DICOMALC()/(_ALC_AUTO) == _TRUE)
                        {
                            // == _ALC_AUTO
                            enumOGCTypeSelect = _OGC_GAMMA5_TYPE + UserGetDICOMALCAmbiSensor();
                        }
                        else
                        {
                            // <  _ALC_AUTO
                            enumOGCTypeSelect = _OGC_GAMMA5_TYPE + (GET_OSD_DICOMALC()%(_ALC_AUTO));
                        }
                        // !!! enumOGCTypeSelect£ºOGC <=10  OCC <=9
                    }
                    else// if(GET_USER_BASE_COLOR_TEMP_TYPE(ucOSDSelectRegion) == _CT_6500)
                    {
                        enumOGCTypeSelect = _OGC_GAMMA25_TYPE;

                        if(GET_OSD_DICOMALC()/(_ALC_AUTO) == _TRUE)
                        {
                            // == _ALC_AUTO
                            enumOGCTypeSelect = _OGC_GAMMA25_TYPE + UserGetDICOMALCAmbiSensor();
                        }
                        else
                        {
                            // <  _ALC_AUTO
                            enumOGCTypeSelect = _OGC_GAMMA25_TYPE + (GET_OSD_DICOMALC()%(_ALC_AUTO));
                        }
                    }
                }
                else if(GET_COLOR_TEMP_TYPE() < _CT_6500)
            ...
            else if(GET_OSD_GAMMA() == _GAMMA_CALIB) // Calib  (IsGammaOptionPCCalibration(GET_OSD_GAMMA()))
            {
                enumOGCTypeSelect = _OGC_GAMMA31_TYPE;
            }
            else
            {
                if(GET_COLOR_TEMP_TYPE() == _CT_USER)
                {
                    ...
                    {
                        enumOGCTypeSelect = _OGC_GAMMA21_TYPE + enumOGCTypeSelect;
                    }
                }
                else if(GET_COLOR_TEMP_TYPE() < _CT_6500)
                {
        ...
        {
            if(GET_OSD_GAMMA() == _GAMMA_DICOM450) // DICOM or DICOM+ALC  (IsGammaOptionDICOMALC(GET_OSD_GAMMA()))
            {
                if(GET_COLOR_TEMP_TYPE() == _CT_USER)
                {
                    if(GET_USER_BASE_COLOR_TEMP_TYPE() == _CT_9300)
                    {
                        ...
                        {
                            // == _ALC_AUTO
                            if(UserGetDICOMALCAmbiSensor() == _ALC_X_RAY)
                                enumOGCTypeSelect = _OGC_GAMMA3_TYPE;
                            else
                                enumOGCTypeSelect = _OGC_GAMMA4_TYPE + UserGetDICOMALCAmbiSensor();
                        }
                        else
                        {
                            // <  _ALC_AUTO
                            if(GET_OSD_DICOMALC() == _ALC_X_RAY)
                                ...
                                enumOGCTypeSelect = _OGC_GAMMA4_TYPE + (GET_OSD_DICOMALC()%(_ALC_AUTO));
                        }
                        // !!! enumOGCTypeSelect£ºOGC <=10  OCC <=9
                    }
                    else// if(GET_USER_BASE_COLOR_TEMP_TYPE(ucOSDSelectRegion) == _CT_6500)
                    {
                        ...
                        if(GET_OSD_DICOMALC()/(_ALC_AUTO) == _TRUE)
                        {
                            // == _ALC_AUTO
                            if(UserGetDICOMALCAmbiSensor() == _ALC_X_RAY)
                                enumOGCTypeSelect = _OGC_GAMMA23_TYPE;
                            else
                                enumOGCTypeSelect = _OGC_GAMMA24_TYPE + UserGetDICOMALCAmbiSensor();
                        }
                        else
                        {
                            // <  _ALC_AUTO
                            if(GET_OSD_DICOMALC() == _ALC_X_RAY)
                                ...
                                enumOGCTypeSelect = _OGC_GAMMA24_TYPE + (GET_OSD_DICOMALC()%(_ALC_AUTO));
                        }
                    }
                }
                else if(GET_COLOR_TEMP_TYPE() < _CT_6500)
                ...
                else if(GET_COLOR_TEMP_TYPE() >= _CT_6500)
                {
                    ...
                    if(GET_OSD_DICOMALC()/(_ALC_AUTO) == _TRUE)
                    {
                        ...
                        if(UserGetDICOMALCAmbiSensor() == _ALC_X_RAY)
                            ...
                            enumOGCTypeSelect = _OGC_GAMMA24_TYPE + UserGetDICOMALCAmbiSensor();
                        //DebugMessageUart("== _ALC_AUTO", enumOGCTypeSelect);
                    }
                    else
                    {
                        // <  _ALC_AUTO
...
#endif
             	DebugMessageUart("DicomALC1/:", GET_OSD_DICOMALC()/(_ALC_AUTO));
             	DebugMessageUart("DicomALC1%:", GET_OSD_DICOMALC()%(_ALC_AUTO));
            }
            else if(GET_OSD_GAMMA() == _GAMMA_CALIB) // Calib  (IsGammaOptionPCCalibration(GET_OSD_GAMMA()))
            {
...
#else
#warning "!!!Please Check Gamma Select"
#endif
    }
 	//DebugMessageUart("OGCTypeSelect:", enumOGCTypeSelect);
#endif

    return enumOGCTypeSelect;
...
BYTE UserCommonAdjustOGCBank(EnumOGCTypeSelect enumOGCTypeSelect, EnumOGCModeSelect enumOGCModeSelect)
{
	...
	if(UserCommonNVRamGetSystemData(_OGCOCC_SEL_BEACON) == _ON)
    {
        ucDivisor_HeaderBank = (BYTE)((65536 - _BEACON_OGC_GAMMA1_ADDRESS) / (DWORD)_BEACON_OGC_GAMMA_TABLE_SIZE);
        ucDivisor = (BYTE)(65536 / (DWORD)_BEACON_OGC_GAMMA_TABLE_SIZE);
     	//DebugMessageUart("Get HeaderBank:", ucDivisor_HeaderBank);
     	//DebugMessageUart("Get ucDivisor:", ucDivisor);
     	//DebugMessageUart("Get OGCType:", enumOGCTypeSelect);

        ucGammaNum = enumOGCTypeSelect;
        enumOGCModeSelect = enumOGCModeSelect;
        ucGammaNum = UserCommonAdjustOGCTypeBeacon(enumOGCTypeSelect);
     	//DebugMessageUart("Get GammaNum:", ucGammaNum);
        ...
        else
        {
     	...
     	DebugMessageUart("Get Bank:", ucQuotient + _BEACON_GAMMA_PARTITION0_FLASH_BANK);

        return ucQuotient + _BEACON_GAMMA_PARTITION0_FLASH_BANK;
    }
    else
#endif //#if(_BEACON_GAMMA_PCM_PARTITION_SUPPORT == _ON)
    {
...
BYTE *UserCommonAdjustOGCAddr(EnumOGCTypeSelect enumOGCTypeSelect, EnumOGCModeSelect enumOGCModeSelect)
{
	...
	if(UserCommonNVRamGetSystemData(_OGCOCC_SEL_BEACON) == _ON)
    {
        ...
        if(ucGammaNum < ucDivisor_HeaderBank)
        {
            pucOutputGammaTypeAddr = g_pucDISP_CALIBRATION_FLASH + _BEACON_OGC_GAMMA1_ADDRESS + ucGammaNum * _BEACON_OGC_GAMMA_TABLE_SIZE;
         	//DebugMessageUart("Get OGCAddr:", _BEACON_OGC_GAMMA1_ADDRESS + ucGammaNum * _BEACON_OGC_GAMMA_TABLE_SIZE);
        }
        else
        {
            if(ucGammaNum < _OGC_GAMMA31_TYPE)
            {
                pucOutputGammaTypeAddr = g_pucDISP_CALIBRATION_FLASH + _BEACON_OGC_GAMMA1_ADDRESS + (ucGammaNum - ucDivisor_HeaderBank) % ucDivisor * _BEACON_OGC_GAMMA_TABLE_SIZE;
             	//DebugMessageUart("Get Index:", (ucGammaNum - ucDivisor_HeaderBank) % ucDivisor );
             	//DebugMessageUart("Get OGCAddr2:", _BEACON_OGC_GAMMA1_ADDRESS + (ucGammaNum - ucDivisor_HeaderBank) % ucDivisor * _BEACON_OGC_GAMMA_TABLE_SIZE);
            }
            else
            {
                if(ucGammaNum == _OGC_GAMMA31_TYPE)
                {
                    pucOutputGammaTypeAddr = g_pucDISP_CALIBRATION_FLASH + _BEACON_OGC_CALIB1_ADDRESS + (ucGammaNum - ucDivisor_HeaderBank) % ucDivisor * _BEACON_OGC_GAMMA_TABLE_SIZE;
                 	//DebugMessageUart("Get Index:", (ucGammaNum - ucDivisor_HeaderBank) % ucDivisor );
                 	//DebugMessageUart("Get OGCAddr3_1:", _BEACON_OGC_CALIB1_ADDRESS + (ucGammaNum - ucDivisor_HeaderBank) % ucDivisor * _BEACON_OGC_GAMMA_TABLE_SIZE);
                }
                else if((ucGammaNum == _OGC_GAMMA32_TYPE)||(ucGammaNum == _OGC_GAMMA33_TYPE))
                {
                    pucOutputGammaTypeAddr = g_pucDISP_CALIBRATION_FLASH + _BEACON_OGC_CALIB2_ADDRESS + (ucGammaNum - ucDivisor_HeaderBank - 1) % ucDivisor * _BEACON_XRITE_CALIB_TABLE_SIZE;
                 	//DebugMessageUart("Get Index:", (ucGammaNum - ucDivisor_HeaderBank - 1) % ucDivisor );
                 	//DebugMessageUart("Get OGCAddr3_2&3:", _BEACON_OGC_CALIB2_ADDRESS + (ucGammaNum - ucDivisor_HeaderBank - 1) % ucDivisor * _BEACON_XRITE_CALIB_TABLE_SIZE);
                }
                else if((ucGammaNum == _OGC_GAMMA34_TYPE)||(ucGammaNum == _OGC_GAMMA35_TYPE))
                {
                    pucOutputGammaTypeAddr = g_pucDISP_CALIBRATION_FLASH + _BEACON_OGC_CALIB4_ADDRESS + (ucGammaNum - ucDivisor_HeaderBank - 3) % ucDivisor * _BEACON_XRITE_CALIB_TABLE_SIZE;
                 	//DebugMessageUart("Get Index:", (ucGammaNum  - ucDivisor_HeaderBank - 3) % ucDivisor );
                 	//DebugMessageUart("Get OGCAddr3_4&5:", _BEACON_OGC_CALIB4_ADDRESS + (ucGammaNum - ucDivisor_HeaderBank - 3) % ucDivisor * _BEACON_XRITE_CALIB_TABLE_SIZE);
                }
            }
        }
...
#if((_OSD_TYPE == _BEACON_2014_OSD)&&(_BEACON_GAMMA_PCM_PARTITION_SUPPORT == _ON))
    UserCommonFlashRead(_BEACON_GAMMA_PARTITION0_FLASH_BANK, (_BEACON_OGC_GAMMA1_ADDRESS + 0x10), 8, pData);
    if((PDATA_WORD(0) == 0xFFFF) || (PDATA_WORD(1) == 0xFFFF) || (PDATA_WORD(2) == 0xFFFF) || (PDATA_WORD(3) == 0xFFFF))   
	{
    	enumOn  = _FUNCTION_OFF;
        DebugMessageUart("No-Gamma Off:",enumOn);
	}
    
    if(GET_COLOR_TEMP_TYPE() == _CT_SRGB)
    {
     	DebugMessageUart("Native-Gamma off:", GET_COLOR_TEMP_TYPE());
    	enumOn  = _FUNCTION_OFF;
    }

    if(IsGammaOptionHybridGamma(GET_OSD_GAMMA()) == _ON)
    {
     	DebugMessageUart("Hybrid-Gamma off:", IsGammaOptionHybridGamma(GET_OSD_GAMMA()));
    	enumOn  = _FUNCTION_OFF;
    }
#endif
