...
bit ScalerSyncProcPreDetect(BYTE ucInputPort);
...
bit ScalerSyncProcIsSeparate(void);
...
void ScalerSyncProcScanInitial(void)
{
    ...
    g_stRegVgaCapture.b1_06_6_ADC_G_B_SWAP = _VGA_INPUT_SWAP_GB;
    g_stRegVgaCapture.b1_06_5_ADC_R_B_SWAP = _VGA_INPUT_SWAP_RB;
    g_stRegVgaCapture.b1_06_4_ADC_R_G_SWAP = _VGA_INPUT_SWAP_RG;
}
...
BYTE ScalerSyncProcTypeAutoRun(void)
{
    ...
    if(ScalerSyncProcIsSeparate() == _TRUE)
    {
...
bit ScalerSyncProcAdcClkInitial(void)
{
    ...
    // APLL & ADC Path thermal Compensation Setting
    ScalerADCThermalCompensationSetting();
