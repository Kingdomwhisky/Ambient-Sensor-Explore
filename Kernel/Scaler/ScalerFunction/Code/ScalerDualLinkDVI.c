...
bit ScalerDualLinkDVIDoubleCheck(EnumSourceSearchPort enumSourceSearchPort, WORD usHwidth, WORD usVHeight)
{
    ...
    if(usVHeight >= usHwidth)    
    {
        // for 1280x1280 1920x1920
        if(usHwidth == usVHeight)
        {
            return _TRUE;
        }        
...
#if(_D2_INPUT_PORT_TYPE == _D2_DUAL_DVI_PORT)
        if((enumSourceSearchPort == _D2_INPUT_PORT) && (ScalerDualLinkDVIRx2GetDualDVIMode() == _TRUE) &&
           ((abs(usHwidth - 1536) <= 4 ) || (abs(usHwidth - (_PANEL_DH_WIDTH / 2)) <= 4 )))
        {
		    DebugMessageMeasure("D2 Dual OK:", usHwidth);
            return _TRUE;
        }
#endif
    }    
