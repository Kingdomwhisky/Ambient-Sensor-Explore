...
void ScalerMDomainMemorySettings(StructSDRAMDataInfo *pstFIFOSize)
{
    ...
    DebugMessageScaler("_DVF_SELECT:", GET_MEMORY_DVF_SELECT());
    ScalerDisplaySetDVFreq();
    DebugMessageScaler("DVFreq:", g_usDVFreq);
    ...
    DebugMessageScaler("MEMORY_SELECT:", GET_MEMORY_SELECT());
        ...
        case _FRAME_SYNC_MEMORY:
...
#endif
            {
...
#if(_RTK_ADVANCED_MOTION_BLUR_REDUCTION_SUPPORT == _ON)
                {
                    ...
                    stPanelInfo.usMaxFreq = g_stDVCustomizedFreq.usMaxFreq;
                    stPanelInfo.usMinFreq = g_stDVCustomizedFreq.usMinFreq;
...
void ScalerMDomainSetDisplayTimingGen(void)
{
...
#if((_PANEL_TYPE == _INNOLUX_LVDS_R213TCE_LU2_2560x2048)||(_PANEL_TYPE == _INNOLUX_LVDS_R213TCE_LU1_2560x2048))
    ScalerSetByte(P0_2B_DISPLAY_FORMAT_DATA_PORT, LOBYTE(GET_MDOMAIN_OUTPUT_VBSTART() + GET_MDOMAIN_OUTPUT_VBHEIGHT() + 0x10));
#else
...
#if((_PANEL_TYPE == _INNOLUX_LVDS_R213TCE_LU2_2560x2048)||(_PANEL_TYPE == _INNOLUX_LVDS_R213TCE_LU1_2560x2048))
    ScalerSetByte(P0_2B_DISPLAY_FORMAT_DATA_PORT, LOBYTE(GET_MDOMAIN_OUTPUT_VSTART() + (GET_MEMORY_SELECT() == _FRAME_SYNC_LINE_BUFFER ? 0x00 : 0x10)));
    ...
    ScalerSetByte(P0_2B_DISPLAY_FORMAT_DATA_PORT, LOBYTE(GET_MDOMAIN_OUTPUT_VSTART() + GET_MDOMAIN_OUTPUT_VHEIGHT() + (GET_MEMORY_SELECT() == _FRAME_SYNC_LINE_BUFFER ? 0x00 : 0x10)));
#else
...
WORD ScalerMDomainODCalculateMemoryUseBW(BYTE ucODbit)
{
    PDATA_DWORD(0) = (((((DWORD)CEILING(_PANEL_DH_WIDTH, _OD_PIXEL_PER_COMP)) * g_stMDomainOutputData.usVTotal * ucODbit / _MEMORY_BUS_WIDTH / 10) * g_usDVFreq / 100) +
                      ((((DWORD)CEILING(_PANEL_DH_WIDTH, _OD_PIXEL_PER_COMP)) * g_stMDomainOutputData.usVTotal * ucODbit / _MEMORY_BUS_WIDTH / 100) * g_usDVFreq * ((UserCommonNVRamGetSystemData(_SSC) / _DPLL_SSCG_DIV_MODE) + _DCLK_SPREAD_RANGE_MARGIN) / 4 / 100 / 10)) * 2;
...
WORD ScalerMDomainGetFRCMemoryBW(EnumFRCBWType enumFRCBWType, WORD usHWidth)
{
    ...
    PDATA_DWORD(2) = ((((DWORD)g_stMDomainOutputData.usVTotal * usHWidth * GET_MEMORY_DATA_BIT() / _MEMORY_BUS_WIDTH / 10) * g_usDVFreq / 100) +
                      (((DWORD)g_stMDomainOutputData.usVTotal * usHWidth * GET_MEMORY_DATA_BIT() / _MEMORY_BUS_WIDTH / 100) * g_usDVFreq * ((UserCommonNVRamGetSystemData(_SSC) / _DPLL_SSCG_DIV_MODE) + _DCLK_SPREAD_RANGE_MARGIN) / 4 / 100 / 10));
...
void ScalerMDomainUpdateInputInfo(void)
{
    ...
    ulIHF = ulIPixelCnt / GET_MDOMAIN_INPUT_VHEIGHT();

    ulIHF = GET_DWORD_MUL_DIV(_MEASURE_CLK, 100, ulIHF);
