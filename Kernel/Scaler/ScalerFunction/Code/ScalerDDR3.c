...
#define _PHASE_CHECK_CLK_COUNT                              (24)
#define _PHASE_CHECK_DQS_DQ_COUNT                           (8)
#define _PHASE_CHECK_DELAY_TAP                              (10)
#endif
...
void ScalerDDR3DCSysSystemBridgeReset(void)
{
    ScalerSetByte(P37_A0_M_DOMAIN_SB_RESET, 0x03);

    // Delay Time us [5,x] Waiting for DDR3 setting
    DELAY_5US();

    // release DCSys
    ScalerSetByte(P37_A0_M_DOMAIN_SB_RESET, 0x01);

    // Delay Time us [5,x] Waiting for DDR3 setting
    DELAY_5US();

    // release Sys Bridge
    ScalerSetByte(P37_A0_M_DOMAIN_SB_RESET, 0x00);
...
bit ScalerDDR3PhaseCheckMCUMode(void)
{
    ...
    if(ScalerTimerPollingFlagProc(_SEARCH_PHASE_POLLING_TIMEOUT, P37_00_MCU_DC_CMD, (_BIT7 | _BIT6 | _BIT5), _FALSE) != _TRUE)
    {
    ...
    // Delay Time us [5,x] Waiting to ensure that read process is finished
    DELAY_5US();
...
void ScalerDDR3ScanPhaseFRCModeDDRClientAccessCtrl(bit bEnable)
{
    if(bEnable == _ENABLE)
    {
        // Enable M1, M2 FRC Capture
        ScalerTimerWaitForEvent(_EVENT_IEN_STOP);
        ScalerFRCEn((_FRC_CAP_MAIN_1 | _FRC_CAP_MAIN_2), _ENABLE);

        // Enable M1_M2 FRC Display
        ScalerFRCEn((_FRC_DISP_MAIN_1 | _FRC_DISP_MAIN_2), _ENABLE);
        ScalerGlobalDDomainDBApply(_DB_APPLY_POLLING);
    }
    else
    {
        // Disable M1, M2, S1 and S2 FRC Capture
        ScalerFRCEn((_FRC_CAP_MAIN_1 | _FRC_CAP_MAIN_2 | _FRC_CAP_SUB_1 | _FRC_CAP_SUB_2), _DISABLE);

        // Disable M1, M2, S1 and S2 FRC Display
        ScalerFRCEn((_FRC_DISP_MAIN_1 | _FRC_DISP_MAIN_2 | _FRC_DISP_SUB_1 | _FRC_DISP_SUB_2), _DISABLE);
        ScalerGlobalDDomainDBApply(_DB_APPLY_POLLING);
    }
}
...
void ScalerDDR3PLLOutputCtrl(bit bEnable)
{
    if(bEnable == _ENABLE)
    {
        ...
        // Delay Time us [45,x] Waiting for DDR3 setting
        DELAY_45US();
    }
    else // Disable
    {
...
bit ScalerDDR3SCheckPhase1(void)
{
    ...
    //***********************************//
    //******DQS01 ,PADCLK,DQ01,REFCMD****//
    //******OESync,Fix00 ,DQ23,DQS23*****//
    //*************RXDelay Tap***********//
    //***********************************//
    ...
    DebugMessageMemoryPhaseCal("***** Clk0 CMD Phase = ", pucInitialCheckPhase[0]);
    DebugMessageMemoryPhaseCal("***** Clk1 DQ01 Phase = ", pucInitialCheckPhase[1]);
    DebugMessageMemoryPhaseCal("***** Clk2 PAD CLK Phase = ", pucInitialCheckPhase[2]);
    DebugMessageMemoryPhaseCal("***** Clk3 DQS01 Phase = ", pucInitialCheckPhase[3]);
    DebugMessageMemoryPhaseCal("***** Clk4 DQS23 Phase = ", pucInitialCheckPhase[4]);
    DebugMessageMemoryPhaseCal("***** Clk5 DQ23 Phase = ", pucInitialCheckPhase[5]);
    ...
    DebugMessageMemoryPhaseCal("Tx Clk Start Good Phase", (pucInitialCheckPhase[2] + ucCheckStartGood) % _TOTAL_CHECK_PHASE_NUM);
    DebugMessageMemoryPhaseCal("Tx Clk End Good Phase", (pucInitialCheckPhase[2] + (ucCheckStartBad - 1)) % _TOTAL_CHECK_PHASE_NUM);
#endif
    ...
    // Check Phase1 Criterion Tx +- 8/Rx +- 5
    if((ucTxClkCheckDiff >= _PHASE_CHECK_CLK_COUNT) && (ucRxCheckDiff >= _PHASE_CHECK_DELAY_TAP))
    {
...
bit ScalerDDR3SCheckPhase2(void)
{
    ...
    // Check Phase2 Criterion Tx Clk +- 8
    ucCheckInitialPhase = ((pucInitialCheckPhase[2] + _TOTAL_CHECK_PHASE_NUM - (_PHASE_CHECK_CLK_COUNT / 2)) % _TOTAL_CHECK_PHASE_NUM);

    for(ucCheckAdrCnt = 0; ucCheckAdrCnt < _PHASE_CHECK_CLK_COUNT; ucCheckAdrCnt++)
    {
        for(ucCheckInteration = 1; ucCheckInteration <= _PHASE_CHECK_ITERATION; ucCheckInteration++)
        {
            ScalerDDR3CheckPhaseFRCModeDDRClientAccessCtrl(_DISABLE);
    ...
    //===========================================
    ucCheckInitialPhase = ((pucInitialCheckPhase[3] + _TOTAL_CHECK_PHASE_NUM - (_PHASE_CHECK_DQS_DQ_COUNT / 2)) % _TOTAL_CHECK_PHASE_NUM);
    ucCheckInitialPhase2 = ((pucInitialCheckPhase[4] + _TOTAL_CHECK_PHASE_NUM - (_PHASE_CHECK_DQS_DQ_COUNT / 2)) % _TOTAL_CHECK_PHASE_NUM);

    // Check Phase2 Criterion Tx DQS +- 4
    for(ucCheckAdrCnt = 0; ucCheckAdrCnt < _PHASE_CHECK_DQS_DQ_COUNT; ucCheckAdrCnt++)
    {
        for(ucCheckInteration = 1; ucCheckInteration <= _PHASE_CHECK_ITERATION; ucCheckInteration++)
        {
            ScalerDDR3CheckPhaseFRCModeDDRClientAccessCtrl(_DISABLE);
    ...
    //===========================================
    ucCheckInitialPhase = ((pucInitialCheckPhase[1] + _TOTAL_CHECK_PHASE_NUM - (_PHASE_CHECK_DQS_DQ_COUNT / 2)) % _TOTAL_CHECK_PHASE_NUM);
    ucCheckInitialPhase2 = ((pucInitialCheckPhase[5] + _TOTAL_CHECK_PHASE_NUM - (_PHASE_CHECK_DQS_DQ_COUNT / 2)) % _TOTAL_CHECK_PHASE_NUM);

    // Check Phase2 Criterion Tx DQ +- 4
    for(ucCheckAdrCnt = 0; ucCheckAdrCnt < _PHASE_CHECK_DQS_DQ_COUNT; ucCheckAdrCnt++)
    {
        for(ucCheckInteration = 1; ucCheckInteration <= _PHASE_CHECK_ITERATION; ucCheckInteration++)
        {
    ...
    ucCheckInitialPhase = ((ucInitialRxDelayTap + _TOTAL_CHECK_PHASE_NUM - (_PHASE_CHECK_DELAY_TAP / 2)) % _TOTAL_CHECK_PHASE_NUM);

    // Check Phase2 Criterion Rx Delay Tap +- 5
    for(ucCheckAdrCnt = 0; ucCheckAdrCnt < _PHASE_CHECK_DELAY_TAP; ucCheckAdrCnt++)
    {
        for(ucCheckInteration = 1; ucCheckInteration <= _PHASE_CHECK_ITERATION; ucCheckInteration++)
        {
            ScalerDDR3CheckPhaseFRCModeDDRClientAccessCtrl(_DISABLE);
