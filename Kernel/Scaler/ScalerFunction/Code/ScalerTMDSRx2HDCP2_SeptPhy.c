...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerTMDSRx2HDCP2_SeptPhy.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __SCALER_TMDS_RX2_HDCP2__
...
BYTE ScalerTMDSRx2Hdcp2RxRsaCalculate_EXINT0(void);
...
{
    // Set AES Cipher Key
    SET_HDCP_2_2_RX2_ONLINE_AES(pucCipherArray, pucInputArray);
...
//--------------------------------------------------
BYTE ScalerTMDSRx2Hdcp2RxRsaCalculate_EXINT0(void) using 1
{
    ...
    if((ScalerGetBit_EXINT(P2D_35_HDCP_I2C_CTRL_7, _BIT0) == _BIT0) && (ScalerGetBit_EXINT(P2D_36_HDCP_I2C_CTRL_8, _BIT0) == _BIT0))
    {
        ...
        switch(ScalerGetByte_EXINT(P2D_22_HDCP_MSGWDP))
        {
            case _HDCP_2_2_AKE_INIT:
                ...
                ScalerHdcp2RxGenRrx_EXINT0(g_pucTmdsRx2Hdcp2Rrx);
            ...
            case _HDCP_2_2_AKE_NO_STORED_KM:
                ...
                // Decrypt Ekpub[km]
                if(ScalerTMDSRx2Hdcp2RxRsaCalculate_EXINT0() == _TRUE)
                {
                    // Decrypt Km by FW to Get 128 bit Km
...
bit ScalerTMDSRx2Hdcp2CaptureKpubKm(void)
{
    ...
    {
        DebugMessageHDCP2("[D2]DCP LLC Signature", g_stRxHdcp2DownLoadKeyType.pucHdcp2KpubKm[usCount]);
    }
#else
