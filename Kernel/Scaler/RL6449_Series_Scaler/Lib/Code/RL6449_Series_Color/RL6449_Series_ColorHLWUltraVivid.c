...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6449_Series_ColorHLWUltraVivid.c
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
void ScalerColorHLWIDLTILSRAdjust(WORD usHPos, WORD usHWidth, WORD usVPos, WORD usVHeight)
{
    ...
    if(bIdomainHLWEn == 1)
    {
        ...
        {
            // even/odd mode
            usIHLWVStart = usIHLWVStart >> 1;
            usIHLWVEnd = usIHLWVEnd >> 1;
        }
        ...
        ScalerWrite(P11_D4_I_DLTI_HLW_HOR_START_HSB_M1 + 0x4000, 8, pData, _AUTOINC);

        ScalerSetBit(P11_B9_SCALER_LSR_HLW + 0x4000, ~_BIT7, _BIT7);
        ScalerSetBit(P11_DC_I_DLTI_HLW_CTRL_M1 + 0x4000, ~_BIT1, _BIT1);
    }
    else
    {
        ...
        ScalerSetBit(P11_B9_SCALER_LSR_HLW + 0x4000, ~_BIT7, 0x00);
        ScalerSetBit(P11_DC_I_DLTI_HLW_CTRL_M1 + 0x4000, ~_BIT1, 0x00);
    }
}
