...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6449_Series_MDomain.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
bit ScalerMDomainMemoryCheck(void)
{
    if((GET_V_SCALE_DOWN() == _TRUE) && (GET_MDOMAIN_OUTPUT_HTOTAL() > (GET_MDOMAIN_OUTPUT_HWIDTH() << 1)))
    {
        ...
        if(ScalerGetBit(P0_0D_WATCH_DOG_CTRL1, _BIT7) == 0x00)
        {
            // Check Input HVsync error flag, Display Vsync timeout flag
            if((ScalerGetBit(P0_02_STATUS0, (_BIT6 | _BIT5)) != 0x00) ||
...
#endif
        {
            // Check fifo overflow and Input HVsync error flag, Display Vsync timeout flag
            if((ScalerGetBit(P0_02_STATUS0, (_BIT6 | _BIT5 | _BIT1)) != 0x00) ||
               (ScalerGetBit(P0_0C_WATCH_DOG_CTRL0, _BIT0) != 0x00))
            {
                return _FALSE;
            }
