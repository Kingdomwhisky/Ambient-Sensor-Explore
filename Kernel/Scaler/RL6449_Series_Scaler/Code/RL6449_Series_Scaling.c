...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6449_Series_Scaling.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
void ScalerScalingUpdateFIFOScalingSize(StructSDRAMDataInfo *pstData)
{
    ...
    if(GET_MEMORY_CONFIG() == _MEMORY_WORK_MODE)
    {
...
#endif
        {
            ...
            ScalerMbrUpdateRtkAdvMbrMDomainInfo(&ulHSDSpace, &g_stMDomainInfo);
#endif
            ...
            if((GET_MEMORY_SELECT() != _FRAME_SYNC_MEMORY) && (ScalerFRCCheckDoubleFrameBuffer(pstData->usInputHWidth, pstData->usInputVHeight) == _TRUE))
            {
                SET_MEMORY_FRAME_BUFFER(_MEMORY_2_FRAME_BUFFER);
