...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6449_Series_TMDSRx3.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6449_SERIES_TMDSRX3__
...
BYTE g_pucTMDSRx3Tap0Max[3];
BYTE g_ucTMDSRx3Tap0Init;
BYTE g_pucTMDSRx3Tap1Max[3];
BYTE g_ucTMDSRx3ClkIRQ;
...
void ScalerTMDSRx3SwitchSlaveAddr(void);
void ScalerTMDSRx3BeforeHotPlugEventProc(void);
void ScalerTMDSRx3AfterHotPlugEventProc(void);
...
void ScalerTMDSRx3SetDFEInitial_EXINT0(void) using 1
{
    ...
    g_ucTMDSRx3Tap0Init = ScalerGetByte_EXINT(P7C_A4_L0_INIT_2);
...
void ScalerTMDSRx3SetPhy_EXINT0(void) using 1
{
    ...
    else  // 3400~6000 Analog CDR
    {
        ...
        // Read Back Lane0
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), 0x00);
        g_pucTMDSRx3Tap0Max[0] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

        // Read Back Lane1
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT6);
        g_pucTMDSRx3Tap0Max[1] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

        // Read Back Lane2
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT7);
        g_pucTMDSRx3Tap0Max[2] = ScalerGetByte_EXINT(P7C_F3_COEF_4);
    ...
    if((ScalerGetBit_EXINT(P7C_E4_GAIN_3, _BIT1) == _BIT1) && (((g_pucTMDSRx3Tap0Max[0] == g_ucTMDSRx3Tap0Init) && (g_pucTMDSRx3Tap0Min[0] == g_ucTMDSRx3Tap0Init)) ||
                                                               ((g_pucTMDSRx3Tap0Max[1] == g_ucTMDSRx3Tap0Init) && (g_pucTMDSRx3Tap0Min[1] == g_ucTMDSRx3Tap0Init)) ||
                                                               ((g_pucTMDSRx3Tap0Max[2] == g_ucTMDSRx3Tap0Init) && (g_pucTMDSRx3Tap0Min[2] == g_ucTMDSRx3Tap0Init))))
    {
        // Disable Sumamp High Gain Mode
        ScalerSetBit_EXINT(P7C_1D_CMU_03, ~_BIT4, 0x00);

        // [1] reg_en_eqen_manual_l0 = 1'b1
        ScalerSetBit_EXINT(P7C_12_ANA_CDR, ~_BIT1, _BIT1);
...
void ScalerTMDSRx3IntHandler_EXINT0(void) using 1
{
    ...
    pucBackUp[3] = ScalerGetByte_EXINT(P72_38_HDMI_CTRL_38);
#endif

    // Freq. Stable Into IRQ
    if(((ScalerGetByte_EXINT(P72_EF_HDMI_ERROR_TH) & 0xC0) == 0xC0) && (g_ucTMDSRx3ClkIRQ == _ENABLE))
    {
    ...
    if(((ScalerGetByte_EXINT(P72_E6_HDMI_FREQDET_CTRL) & 0x06) == 0x06) && (g_ucTMDSRx3ClkIRQ == _ENABLE))
    {
        // Clear Unstable IRQ Flag
        ScalerSetBit_EXINT(P72_E6_HDMI_FREQDET_CTRL, ~_BIT2, _BIT2);
        ...
        if((abs(g_usTMDSRx3ClkCount - ScalerTMDSRx3MeasureClk_EXINT0(_CLK_SOURCE_TMDS)) > (g_usTMDSRx3ClkCount >> 6)) ||
           (abs(GET_INPUT_PIXEL_CLK() - ((GET_TMDS_OFFLINE_MEASURE_CLK() << 3) / GET_COLOR_DEPTH())) > (GET_INPUT_PIXEL_CLK() >> 6)))
        {
            ...
            ScalerSetBit_EXINT(P72_01_HDMI_CTRL_01, ~_BIT3, _BIT3);

            // Disable Scrambling Status
            ScalerSetByte_EXINT(P72_39_HDMI_CTRL_39, _P72_3A_PT_21_PORT_PAGE72_HDMI_SCDC_21);
            ScalerSetBit_EXINT(P72_3A_HDMI_CTRL_3A, ~_BIT0, 0x00);
    ...
    {
        // clear AKSV flag
        ScalerSetBit_EXINT(P72_A9_TMDS_ABC0, ~(_BIT7 | _BIT6 | _BIT5 | _BIT2), _BIT7);
...
#if(_D3_HDMI_2_0_SUPPORT == _ON)
    ScalerSetByte_EXINT(P72_38_HDMI_CTRL_38, pucBackUp[3]);
...
WORD ScalerTMDSRx3MeasureClk(BYTE ucMeasureClk)
{
    ...
    ScalerSetDataPortBit(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0, ~(_BIT5 | _BIT4 | _BIT3), 0x00);

    // Select reference clk to Xtal Clk and measure clk
    ScalerSetBit(P72_AD_TMDS_CPS, ~(_BIT6 | _BIT5 | _BIT4), ((ucMeasureClk << 5) | _BIT4));

    // Start Measure
    ScalerSetDataPortBit(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0, ~(_BIT5 | _BIT4 | _BIT3), _BIT3);
...
#if((_D3_HDMI_2_0_SUPPORT == _ON) && (_HDMI_2_0_YCBCR420_SUPPORT == _ON))
    BYTE ucRatio = 1;
#endif
...
BYTE ScalerTMDSRx3TMDSScanInputPort(BYTE ucInputType)
{
    ...
    if(GET_TMDS_RX3_PHY_SET() == _TRUE)
    {
        ...
        if(ucTMDSType == _SOURCE_HDMI)
        {
            ...
            if((GET_COLOR_DEPTH() != _COLOR_DEPTH_8_BITS) && (GET_COLOR_SPACE() != _COLOR_SPACE_YCBCR422))
            {
                ...
                if(GET_COLOR_SPACE() == _COLOR_SPACE_YCBCR420)
                {
                    ...
                    ScalerSetBit(P72_AD_TMDS_CPS, ~_BIT7, _BIT7);
                }
                else
                {
                    // HDMI Output Div2 Disable
                    ScalerSetBit(P72_AD_TMDS_CPS, ~_BIT7, 0x00);
                }
            }
            else
            {
        ...
        // solve philip DVD issue
        if(ScalerGetBit(P7C_E4_GAIN_3, _BIT1) == _BIT1)
        {
            ...
            // [1] reg_en_eqen_manual_l0 = 1'b0
            ScalerSetBit(P7C_12_ANA_CDR, ~_BIT1, 0x00);

            // Enable Sumamp High Gain Mode
            ScalerSetBit(P7C_1D_CMU_03, ~_BIT4, _BIT4);
        }
...
bit ScalerTMDSRx3TMDSStableDetect(BYTE ucSourceType)
{
    ...
    {
        if((ScalerGetBit(P72_A9_TMDS_ABC0, _BIT6) == _BIT6) && (!((((ScalerGetBit(P72_A9_TMDS_ABC0, _BIT7)) == _BIT7) || ((ScalerGetBit(P72_AE_TMDS_RPS, _BIT1)) == _BIT1)) && ((ScalerGetBit(P72_A9_TMDS_ABC0, _BIT5)) == _BIT5))))
        {
