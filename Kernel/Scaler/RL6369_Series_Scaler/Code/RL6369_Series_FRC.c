...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6369_Series_FRC.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
#define __RL6369_SERIES_FRC__
...
void ScalerFRCInputCtrl(WORD usHorSize, WORD usVerSize)
{
    ...
    // Frame mode
    ulTotalSize = ScalerMemoryCalculateFrameLineTotalPixel(_FRAME_MODE, usHorSize, usVerSize);

    // Set Frame mode
    ScalerSetBit(P5_B7_IN1_LINE_NUM_H, ~_BIT4, _BIT4);

    stStep.ulBlockStep = ScalerMemoryAddressConversion(ulTotalSize);
