...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6369_Series_ColorColorConversionUltraVividSupport.c
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
//****************************************************************************
void ScalerColorSpaceConvertUltraVividSupport(EnumColorSpace enumColorSpace, EnumDBApply enumDBApply);
...
void ScalerColorSpaceConvertUltraVividSupport(EnumColorSpace enumColorSpace, EnumDBApply enumDBApply)
{
    ...
    ScalerColorSpaceLoadYuv2RgbTable(enumColorSpace, _ON);
    ...
    if(GET_COLOR_SPACE_RGB(enumColorSpace) == _TRUE)
    {
        ...
        else
        {
            if(GET_COLOR_RGB_LIMIT_RANGE_USER() == _TRUE)
            {
                ...
                ScalerSetBit(P14_A1_SR_YCC2RGB_CTRL, ~(_BIT4 | _BIT0), (_BIT4 | _BIT0));
            }
            else
            {
                // Disable D-Domain YUV2RGB Identity_Matrix
                ScalerSetBit(P14_A1_SR_YCC2RGB_CTRL, ~(_BIT4 | _BIT0), 0x00);
            }
        }
    }
    else
