...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6492_Series_QC_Color_IMDomain.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6492_SERIES_QC_COLOR_IMDOMAIN__
...
BYTE code t422to444_Table0[] = {
    0x90, 0x9A, 0xCE, 0x70, 0xAC, 0x4E, // M1
};

BYTE code t422to444_Table1[] = {
    0x3E, 0x3A, 0xBF, 0x5B, 0xB1, 0xE4, // M1
};

BYTE code t422to444_Table2[] = {
    0x52, 0x2D, 0x60, 0x24, 0x00, 0x56, // M1
};
...
BYTE code tQC_IDITHER_CRCTable_P2[] =
{
    0x2C, 0x77, 0x5C, 0x00, 0x00, 0x00, // M1
};
...
bit ScalerQC422to444Test(void)
{
    ...
    DebugMessageQC("5. Mode_0", 0x00);
    ucFlag = ucFlag | ((BYTE)ScalerQCCRCCompare(_I_DOMAIN_CRC, _IMD_CRC_I_AFTER_VGIP, _M1, t422to444_Table0, 1) << 0);

    // 1.
    ScalerSetByte(P31_40_YUV422_TO_444_PATH0, 0x0B);
    DebugMessageQC("5. Mode_1", 0x00);
    ucFlag = ucFlag | ((BYTE)ScalerQCCRCCompare(_I_DOMAIN_CRC, _IMD_CRC_I_AFTER_VGIP, _M1, t422to444_Table1, 1) << 1);

    // 2.
    ScalerSetByte(P31_40_YUV422_TO_444_PATH0, 0x0E);
    DebugMessageQC("5. Mode_2", 0x00);
    ucFlag = ucFlag | ((BYTE)ScalerQCCRCCompare(_I_DOMAIN_CRC, _IMD_CRC_I_AFTER_VGIP, _M1, t422to444_Table2, 1) << 2);
...
bit ScalerQCIDitherTest(void)
{
    ...
    ScalerSetBit(P0_20_M1_DUMMY2, ~(_BIT7), _BIT7);
    ucData = ((BYTE)ScalerQCCRCCompare(_I_DOMAIN_CRC, _IMD_CRC_I_AFTER_IDITHER, _M1, tQC_IDITHER_CRCTable_P2, 1) << 1);

    ScalerSetByte(P0_89_I_DITHER_COMMON_CTRL1_SETA, 0x00);
    ScalerQCIDomainRandomGenOff();
    ScalerSetBit(P0_20_M1_DUMMY2, ~(_BIT7), 0x00);
