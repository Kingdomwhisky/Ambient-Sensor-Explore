...
void ScalerDpTxMstHandler(void)
{
    ...
    {
#if(_DP_MST_AUTO_SEARCH_SUPPORT == _ON)
        if((GET_DP_RX_SELF_GEN_DOWN_REQUEST_MSG() == _FALSE) && (GET_DP_TX_RECEIVED_MSG_COUNT() == 0))
#endif
        {
            return;
        }
                ...
                case _DP_MSG_CONNECTION_STATUS_NOTIFY:
                    ...
                    else
                    {
                        ...
                        if(GET_DP_RX_REPORT_DOWNSTREAM_DP_CABLE_STATUS() == _TRUE)
                        {
                            ...
                            ScalerTimerActiveTimerEvent(SEC(8), _SCALER_TIMER_EVENT_DP_TX_FORCE_REBUILD_TOPOLOGY_EVENT);
                        }
                        else
                        {
                            // Set Rx Sink Count
...
bit ScalerDpTxMstDownRequestMsgAnal(void)
{
    ...
    // Tx Bypass Case
    if((GET_REG_DP_RX_DOWN_REQUEST_MSG_LCR() > 0) && (GET_DP_RX_SELF_GEN_DOWN_REQUEST_MSG() == _FALSE))
    {
...
bit ScalerDpTxMstSyncTableProcess(BYTE ucPayloadID, WORD usPBN)
{
    ...
    BYTE ucStreamNum = ScalerDpRxMstSearchMatchStream(ucPayloadID) + 1;

    // Find which ST Source Is assigned to the STx Containing current payload ID
    if(ucStreamNum == ScalerDpRxMstStreamMuxCheck(_DP_ST_SOURCE3))
    {
        enumTxCurrentSTX = _DP_ST_SOURCE3;
    }
    else if(ucStreamNum == ScalerDpRxMstStreamMuxCheck(_DP_ST_SOURCE4))
    {
        enumTxCurrentSTX = _DP_ST_SOURCE4;
    }
    else if(ucStreamNum == ScalerDpRxMstStreamMuxCheck(_DP_ST_SOURCE5))
    {
...
void ScalerDpTxMstQueryEncryptionWriteSha256Input(bit bRepeater)
{
    ...
    // Load Riv (64-bits)
    ScalerDpTxAuxNativeRead(0x06, 0x93, 0x28, 8, &pucHmacSha256Input[10]);
