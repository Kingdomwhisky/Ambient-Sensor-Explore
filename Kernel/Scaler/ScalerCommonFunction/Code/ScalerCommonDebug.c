...
void ScalerUartDebugMessage(BYTE *pucArray1, DWORD ulArray2);
#endif
...
#if(_UART_SUPPORT == _ON)
#define _UART_NUM_SIZE                        10  

//--------------------------------------------------
// Description  : UART Debug Message process
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerUartDebugMessage(BYTE *pucArray1, DWORD ulArray2)
{
	BYTE ucCount,ucIndex=0;
	BYTE UartNum[_UART_NUM_SIZE] = {0};

	do
	{
		ScalerMcuUartWrite(*pucArray1);
	}while(((*pucArray1++)!=0)&&((ucIndex++)< 240));

#if 0
	{	
    	// dec  -> 4294967295:hex - >[0xFFFFFFFF]
        // UartNum[0~9]
    	for(ucIndex = 0; ucIndex < _UART_NUM_SIZE; ucIndex++)
    	{
    	   UartNum[ucIndex] = ulArray2%10;
    	   ulArray2 = ulArray2/10;
    	}

    	for(ucIndex = 9; ucIndex > 0; ucIndex--)
    	{
    		if(UartNum[ucIndex] != 0) 
            break;
    	}
   
		for (ucCount = 0; ucCount <= ucIndex; ucCount++) 
		{
			ScalerMcuUartWrite(UartNum[ucIndex - ucCount] + '0');
		}
	}
#else
	{	
    	...
    	{
    	   UartNum[ucIndex] = ((ulArray2 >> (4 * ucIndex)) & 0x0F);
    	}
		...
		for (ucCount = ucIndex + 1; ucCount > 0; ucCount--) 
		{
			if (UartNum[ucCount - 1] > 9)
    			ScalerMcuUartWrite(UartNum[ucCount - 1] - 10 + 'A');
			else
    			ScalerMcuUartWrite(UartNum[ucCount - 1] + '0');
		}
	}
#endif
