...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6463_Series_ODFunction.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
void ScalerODCompressionSetting(void)
{
    ...
    if(GET_OD_USE_BIT() == _OD_SUPERPIXEL1X2_75BIT_BIT_NUM)
    {
        ...
        // Current path: enable compression(average mode), set to Super-pixel 1x2
        ScalerSetBit(P3_B0_LS_FRAME0, ~(_BIT4 | _BIT3 | _BIT2 | _BIT1), (_BIT4 | (_OD_SPR_PIXEL_1X2 << 1)));

        // Enable previous/current frame decompression(duplication mode)
        ScalerSetBit(P3_B1_LS_FRAME1, ~(_BIT7 | _BIT6 | _BIT3 | _BIT2), (_BIT7 | _BIT6 | _BIT3 | _BIT2));

        // Set current frame source pixel resolution: 5bit
        ScalerSetBit(P3_B2_LS_FRAME2, ~(_BIT7 | _BIT6), (_OD_SRC_PIXEL_RESOL_5 << 6));
    ...
    else if(GET_OD_USE_BIT() == _OD_SUPERPIXEL1X4_60BIT_BIT_NUM)
    {
        ...
        // Current path: enable compression(average mode), set to Super-pixel 1x4
        ScalerSetBit(P3_B0_LS_FRAME0, ~(_BIT4 | _BIT3 | _BIT2 | _BIT1), (_BIT4 | (_OD_SPR_PIXEL_1X4 << 1)));

        // Enable previous/current frame decompression(duplication mode)
        ScalerSetBit(P3_B1_LS_FRAME1, ~(_BIT7 | _BIT6 | _BIT3 | _BIT2), (_BIT7 | _BIT6 | _BIT3 | _BIT2));

        // Set current frame source pixel resolution: 8bit
        ScalerSetBit(P3_B2_LS_FRAME2, ~(_BIT7 | _BIT6), (_OD_SRC_PIXEL_RESOL_8 << 6));
