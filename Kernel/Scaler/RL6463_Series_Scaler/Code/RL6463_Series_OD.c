...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6463_Series_OD.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6463_SERIES_OD__
...
void ScalerODEnable(EnumFunctionOnOFF enumOn)
{
    ...
    // Enable OD
    //=================
    if((enumOn == _FUNCTION_ON) && (GET_OD_USE_BIT() != _OFF) && (GET_MEMORY_CONFIG() != _MEMORY_WORK_MODE))
    {
        // Set memory FIFO MUX to OD
        ScalerSetBit(P3_C9_LS_FIFO_CTRL0, ~_BIT7, 0x00);
    ...
    else
    {
        ...
        // After turning off OD memory access, set memory FIFO MUX to FRC
        ScalerTimerWaitForEvent(_EVENT_DVS); // OD ctrl bit DB effective at DVS falling
        ScalerTimerWaitForEvent(_EVENT_DVS); // Last OD read process is finished
