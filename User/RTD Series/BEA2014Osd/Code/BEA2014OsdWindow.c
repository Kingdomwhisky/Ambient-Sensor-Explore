...
// Output Value :
//--------------------------------------------------
void OsdWindowDrawingByPixel(BYTE ucWindow, WORD ucRow, WORD ucCol, WORD ucWidth, WORD ucHeight, BYTE ucColor)
{
    WORD usHstart,usHend,usVstart=0,usVend;

    usHstart = ucCol;
    usHend = usHstart + ucWidth;

    if(ucRow > 0)
    {
        usVstart = ucRow;
    }
    usVend = usVstart + ucHeight;
